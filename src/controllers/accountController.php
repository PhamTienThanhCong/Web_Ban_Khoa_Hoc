...
class account extends Controllers
{
    ...
    public function update_account_admin()
    {
        ...
        if ($_FILES["image"]["name"]){
            $image = $_FILES["image"];
            require_once "./src/controllers/save_image.php";
        }
        $delete_image;
        if ($uploadOk == 1){
            $change_account = $this->model('admin');
            $result = $change_account->updateAccount($id,$name,$email,$info,$password,$name_image,$target_dir);
            if ($result != "1"){
        ...
        header("Location: ".$actual_link."/".$link."/manage_account");
