...
class admin extends ConnectDB
{
    ...
    public function updateAccount($id, $name, $email, $info, $password, $name_image,$target_dir)
    {
        $sql = "SELECT * FROM `admin` WHERE `id_admin` = '$id'";
        $account = mysqli_query($this->connection, $sql);
        $account = mysqli_fetch_array($account);
        if (isset($account['password'])) {
            $verify = password_verify($password, $account['password']);
            if ($verify) {
                $delete_image = $account['image_admin'];
                if ($name_image == ""){
                    $name_image = $account['image_admin'];
                    $delete_image = "1";
                }
                if ($email != $account['email_admin']){
                    $sql = "SELECT COUNT(*) as checkmail FROM `admin` WHERE `email_admin` = '$email'";
                    $account = mysqli_query($this->connection, $sql);
                    $account = mysqli_fetch_array($account);
                    if ($account['checkmail'] != 0) {
                        die(0);
                    }
                }
                if ($delete_image != "avatar.jpg" && $delete_image != "1"){
                ...
                $sql = "UPDATE
                        ...
                        SET
                            `name_admin` = '$name',
                            `email_admin` = '$email',
                            `image_admin` = '$name_image',
                            `description_admin` = '$info'
                        WHERE
                            `id_admin` = '$id'";
                mysqli_query($this->connection, $sql);
                $_SESSION['name_admin'] = $name;
                $_SESSION['image_admin'] = $name_image;
                return $delete_image;
            }
            die("fall pass");
        }
